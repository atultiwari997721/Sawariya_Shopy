...
export default function StoreManageProducts() {
  ...
  const fetchProducts = async () => {
    try {
      ...
      setProducts(data.products.sort(( a,b) => new Date(b.createdAt) - new 
        Date(a.createdAt)))
    } catch (error) {
              ...
              <td className="px-4 py-3">
                <div className="flex gap-2 items-center">
                  <Image
                    ...
                    alt=""
                  />
                  {product.name || <span className="text-red-500">No Name</span>}
                </div>
              </td>
              <td className="px-4 py-3 max-w-md text-slate-600 hidden md:table-cell truncate">
                {product.description || <span className="text-red-500">No Description</span>}
              </td>
